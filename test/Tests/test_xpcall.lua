...
function test_xpcall(Logging)
	...
	util.assertCompare("Logger xpcall return values do not match", expectedReturnValues, {select(2, unpack(actualReturnValues))})
	...
	assert(util.matchEnd(errorActual, errorMessage), ("Logger:xpcall should call the error handler with the error message if the function raises an error, expected %q, got %q"):format(errorMessage, errorActual))
