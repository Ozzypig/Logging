...
function test_pcall(Logging)
	...
	assert(
		...
		"Logger:pcall should return true if the function runs without error: " .. tostring(actualReturnValues[2])
	)
	util.assertCompare(
		"Logger:pcall return values do not match",
	...
	assert(
		util.matchEnd(actualReturnValues[2], errorMessage),
		("Logger:pcall should return the error when the function raises one, expected %q got %q"):format(
			actualReturnValues[2],
			errorMessage
		)
	)
end
